...
| **Chat Logic & Prompting** | Setup the LLM prompt to include both the transcript and the conversation as context for user questions. | 2 |
| **Transcription Service** | Audio processing with ffmpeg and yt-dlp. | 1 |
...
| **Logging Levels** | Creating logs based on the exception, error and process of the app.| 2 |
...
| **Modular Component Build** | Build `VideoInput`, `ChatPanel`, and `TranscriptPanel` as standalone components. | 2 |
| **Custom Hooks Logic** | Implement `useVideoChat` and `useChat` to handle API calls and message states with local storage. | 3 |
| **Input & Validation** | Develop `validators.js` for URL checking and `ProcessingIndicator` for feedback. | 1 |
| **Error Boundary** | Develop `errorBoundary.js` for URL checking and wrapped it around components | 2 |
| **State & Context Sync** | Ensure `TranscriptPanel` and `ChatPanel` correctly share the video context. | 2 |
- **Subtotal Development Duration:**  **27 hrs**
...
| Task | Description | Est. Hours |
| :--- | :--- | :--- |
| **Unit Testing** | Testing all the smaller parts | 2 |
| **Automation** | Testing performances of the web application with increasing capabilities | 3 |
| **Hardware Optimization** | Test VRAM consumption on Colab to ensure the `base` model runs without crashing. | 2 |
| **Edge Case Handling** | Logic to handle invalid URLs, very long videos, or network timeouts. | 2 |
...
| **Functional E2E Test** | Conduct "perfect path" testing. | 1 |
| **Refining Frontend** | Refining as a whole before productions | 4 |
| **Refining Backend** | Refining as a whole before productions | 4 |
| **Subtotal Testing Duration** | | **20 hrs** |
...
| **Monitoring/Logging** | Create a mean to mornitor the application lifetime | 3 |
| **Subtotal Deployment Duration** | | **9  hrs** |
...
> **Timeline:** With ~21 hours per week, this project is estimated for completion in **2~ weeks**.
